...
SELECT l.`id` AS offset_id, u.`id`, u.`about`, u.`followers`, u.`following`, u.`posts`, u.`website`, u.`country_id`, u.`posts`, u.`avatar`, u.`last_active`, u.`username`, u.`fname`, u.`lname`, u.`email`, u.`verified`, u.`follow_privacy` FROM `<?php echo($data['t_likes']); ?>` l
